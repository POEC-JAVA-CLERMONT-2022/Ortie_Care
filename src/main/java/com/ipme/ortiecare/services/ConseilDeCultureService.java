...
public class ConseilDeCultureService {
	...
	public ConseilDeCultureDTO findById(UUID id) {
		if (id != null && id.toString() != "") {
			...
			logger.warn(
					"La recherche par id n'a pas fonctionné, renvoi d'un objet vide conseil de culture. Valeur du paramètre passé : "
							+ id);
			return new ConseilDeCultureDTO();
		}
	}
	...
	public ConseilDeCulture create(String titre, String description) {
		if (titre != null && description != null && titre != "" && description != "") {
			...
			logger.error(
					"Appel de la methode create de Conseil de culture ; un des paramètres est null ou vide ; Renvoi d'un objet vide");
